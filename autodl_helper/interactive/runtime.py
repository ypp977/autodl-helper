...
def _capture_managed_loggers() -> list[logging.Logger]:
    managed: list[logging.Logger] = [logging.getLogger()]
...
def _install_thread_log_filters(thread_filter: logging.Filter) -> tuple[list[logging.Logger], list[logging.Handler]]:
    managed_loggers = _capture_managed_loggers()
    ...
    return managed_loggers, managed_handlers


def _remove_thread_log_filters(
    thread_filter: logging.Filter,
    managed_loggers: list[logging.Logger],
    managed_handlers: list[logging.Handler],
) -> None:
    seen_handlers: set[int] = set()
    for logger in managed_loggers:
        with contextlib.suppress(Exception):
            logger.removeFilter(thread_filter)
        for handler in logger.handlers:
...
def _capture_callable_output(action: Callable[[], Any]) -> tuple[Any, str]:
    ...
    suppress_current_thread = _ThreadFilter(thread_id)
    managed_loggers: list[logging.Logger] = []
    managed_handlers: list[logging.Handler] = []
    try:
        ...
        managed_loggers, managed_handlers = _install_thread_log_filters(suppress_current_thread)
        root_logger.addHandler(capture_handler)
        result = action()
    finally:
        root_logger.removeHandler(capture_handler)
        capture_handler.close()
        _remove_thread_log_filters(suppress_current_thread, managed_loggers, managed_handlers)
        with _stream_proxy_lock:
