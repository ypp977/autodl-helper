...
当前版本的主菜单“刷新状态”、Keeper“立即执行”、账户“健康检查”都是后台提交任务，不阻塞输入，并会在任务完成后自动重绘页面。
...
- 看板默认读取本地 SQLite 历史，所以打开 UI 和普通重绘应尽量快。
- 主菜单“刷新状态”会访问 AutoDL 官方接口，完成后看板会显示“刚刚刷新”或“刷新失败”。
- Keeper“立即执行”和账户“健康检查”也会访问 AutoDL 官方接口，完成后自动回显摘要。
- 页面显示“刷新中”时，代表任务已提交到后台，当前仍可能展示本地 SQLite 状态。
