...
from autodl_helper.services import launchd as service_launchd
...
def test_build_launch_agent_plist_contains_expected_paths(tmp_path):
    ...
    assert '<string>autodl-helper</string>' in plist
...
def test_install_launch_agent_writes_plist_without_loading(tmp_path):
    ...
    assert '<string>autodl-helper</string>' in content
    assert '<string>run</string>' in content
    assert '<string>daemon</string>' in content
    assert str(tmp_path / 'config.yaml') in content


def test_install_launch_agent_defaults_to_current_python_module(tmp_path):
    plist_path = service_launchd.install_launch_agent(
        config_path=str(tmp_path / 'config.yaml'),
        home_dir=tmp_path,
    )
    ...
    assert '<string>autodl_helper</string>' in content
    assert '<string>run</string>' in content
    assert '<string>daemon</string>' in content
